...
Modifiez les opérateurs `@` par `==` (égal) ou `!=` (différents) pour que tous les `assert` passent.
...
a = [1, 2]      # Créez une liste 
b = a           # b pointe vers le même objet que a
a += [2]        # ajoute 2 à la fin de a
# Les lists sont des types mutables, donc l'objet pointé par 
# a à simplement été modifié en mémoire. a et b pointe donc 
# toujours vers le même objet.
assert id(a) @ id(b) 

a = (1, 2)      # Créez un tuple 
b = a           # b pointe vers le même objet que a
a += (2,)       # ajoute 2 à la fin de a
# Les tuples sont des types immutables, donc pour ajouter
# 2 à la fin de a il faut créer un nouveau tuple avec l'élément
